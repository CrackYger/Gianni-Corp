...
export default function Finance(){
  ...
  const monthlySeries = useMemo(()=>{
    const byMonth: Record<string,{income:number;expense:number;net:number}> = Object.fromEntries(monthKeys.map(k=> [k,{ income:0, expense:0, net:0 }]));
  ...
  const expenseByService = useMemo(()=>{
    ...
    return Array.from(map.entries()).map(([name, amount])=> ({ name, amount })).sort((a,b)=> b.amount - a.amount).slice(0, topExp);
...
const expenseByService = useMemo(()=>{
  ...
  useEffect(()=>{(async()=>{ try { setServices(await _db2.services.toArray()); } catch{} })()},[]);
  ...
  const toolbar = (
    <div className="toolbar">
      <input aria-label="Monat" className="input w-[150px]" type="month" value={month} onChange={e=>setMonth(e.target.value)} />
